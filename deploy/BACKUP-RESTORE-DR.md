...
2. **Tenant isolation intact** — log in to two tenants; confirm each sees only its own
   data (RLS active: `SELECT relforcerowsecurity FROM pg_class` = true on tenant tables).
...
4. **Signature manifest + row counts** — spot-check `signature_record` and key table
   counts against the pre-incident figures recorded in the backup manifest.
5. **File integrity** — a sample of `file_reference` rows resolve to files whose
