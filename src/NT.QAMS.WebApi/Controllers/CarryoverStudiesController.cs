...
public sealed class CarryoverStudiesController(ISender sender) : ControllerBase
{
    ...
    public async Task<IActionResult> Signatures(Guid id, CancellationToken ct) =>
        Ok(await sender.Send(new NT.QAMS.Application.ComplianceLedger.GetSignaturesForSubjectQuery($"CAR:{id:N}"), ct));
    ...
    {
        await sender.Send(new SignOffCarryoverCommand(id, request.Password, request.Pin), ct);
        return NoContent();
    }
}
