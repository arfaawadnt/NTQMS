...
public sealed record SignOffInterferenceCommand(Guid StudyId, string Password, string Pin) : ICommand;

public sealed class InterferenceWorkflowHandlers(
    ...
    ICommandHandler<SignOffInterferenceCommand>
{
    ...
    public async Task Handle(SignOffInterferenceCommand c, CancellationToken ct)
    {
        ...
        if (s.State != InterferenceState.Calculated)
        {
            throw new InvalidStateTransitionException(
                "INT-013", $"Only a calculated study can be signed off (current: {s.State}).");
        }

        var subjectRef = $"INT:{s.Id:N}";
        await signatures.SignAsync(
            actor, c.Password, c.Pin, "Signed off interference study", subjectRef,
