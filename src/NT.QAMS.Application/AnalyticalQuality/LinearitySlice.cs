...
public sealed record SignOffLinearityCommand(Guid StudyId, string Password, string Pin) : ICommand;

public sealed class LinearityWorkflowHandlers(
    ...
    ICommandHandler<SignOffLinearityCommand>
{
    ...
    public async Task Handle(SignOffLinearityCommand c, CancellationToken ct)
    {
        ...
        // Pre-validate SoD + state BEFORE minting — the signature ledger is append-only, so a
        // signature must never exist for a sign-off that then fails its gates (mirrors the NC
        // verify pilot / DocumentCommands). The aggregate re-checks both invariants.
        ...
        if (study.State != LinearityState.Calculated)
        {
            throw new InvalidStateTransitionException(
                "LIN-012", $"Only a calculated study can be signed off (current: {study.State}).");
        }

        var subjectRef = $"LIN:{study.Id:N}";
        await signatures.SignAsync(
            actor, c.Password, c.Pin, "Signed off linearity study", subjectRef,
