...
| URS-129 | A **closed** nonconformance shall be re-openable by an authorized signer for further corrective action (ISO/IEC 17025 §8.7). Re-opening shall require a **documented reason** and the actor's electronic signature (account password + PIN, §11.200(a)(1)), recorded as an immutable signature bound to the record and the reason (§11.50/§11.70); the state gate shall be checked **before** any signature is minted; on success the NC returns to the action-plan stage so its CAPA work can be revisited and re-verified. | Domain `Nonconformance.Reopen(reason, actorId)` — guarded `Closed → ActionPlan` (NC-023 wrong state, NC-024 missing reason), `ReopenReason` (text, bound in the validator), raises `NcReopened(NcId, NcRef, ReopenedBy, Reason)`; NC is deliberately **not** in the signed-record immutability trigger set, so the transition is a legitimate audited state change. `ReopenNcCommand(NcId, Reason, Password, Pin)` gated `[RequirePermissionPolicy(nc, Sign)]` + `ReopenNcValidator`; `ReopenNcHandler` pre-validates `Status==Closed` then `IESignatureService.SignAsync` (subject `NC:{id}`, meaning `"Re-opened {ref}: {reason}"`, `SignatureContentHash.Compute(nc, action=reopen, reason)`) then `Reopen`; controller `POST /api/nonconformances/{id}/reopen` → `[RequirePermission(nc, Sign)]`; contract `ReopenNcRequest`; migration `AddNcReopenReason` (nullable `text`, round-trips). Frontend: shared `EsignDialogComponent` gains an optional mandatory-reason field (backward-compatible — existing verify/close callers unchanged); NC-detail Closed-state re-open action gated `nc.sign`, refreshing the §11.50 manifest on success. **Reuses `nc.sign` — no new catalogue key, no tenant upgrade gap.** | AUTO — `ReopenNcSigningTests` (3, driving the **real** `ESignatureService`: a valid credential re-opens the NC to ActionPlan and mints exactly one manifest entry whose meaning carries the reason; wrong PIN → SIG-001 + zero signatures + NC still Closed; wrong state → NC-023 + zero signatures); domain `NonconformanceTests` +3 (re-open → ActionPlan + reason + `NcReopened`; reason required NC-024; only-from-Closed NC-023); `CommandPolicyTests` green; `ApiSurface.approved.txt` +2 lines reviewed. Frontend production build clean; the extended `qams-esign-dialog` verified **live** (dev, 2026-08-08) to be backward-compatible — the NC verify caller renders password + PIN and **no** reason field. | Template — engineering complete; automated + partial live evidence recorded; witnessed positive-reopen OQ pending (QA execution — the dev dataset holds no Closed NC and manufacturing one needs a PIN-holding signer other than the raiser) |
| URS-130 | The Quality Analytics dashboard shall be exportable as a **professional, comprehensive** report in **PDF** and **Excel**: the composite Quality Health Score (gauge + weighted per-category progress bars), each sub-system's KPIs, its Pareto/breakdown bars, and the 5×5 risk heat-matrix. The export shall re-compute under the caller's own view permissions and branch/department scope (a section the caller cannot see is absent from the report), be gated by `reports.export`, carry the 21 CFR Part 11 §11.10(b) provenance stamp, and be recorded as a `RECORD_EXPORTED` security event. | New `IExportService.ToQualityAnalyticsReportPdf` / `ToQualityAnalyticsReportXlsx` (`src/NT.QAMS.Infrastructure/Exports/ExportService.QualityAnalytics.cs`, QuestPDF + ClosedXML) over a `QualityAnalyticsReportPack` wrapping the structured `QualityAnalyticsDto` — the PDF draws a brand band, a score gauge, per-component progress bars, per-section KPI tiles, Pareto bars and a risk heat-matrix; the XLSX is a health-score summary sheet plus one sheet per sub-system (KPIs + breakdown + records), with the existing `GuardFormulaLike` CSV-injection defence. `GET /api/exports/quality-analytics.pdf|.xlsx` `[RequirePermission(reports, Export)]` re-query `GetQualityAnalyticsQuery(branchId, departmentId)`, resolve a human scope line, and `LogExportAsync`. Frontend: Export PDF / Excel buttons on `quality-analytics.component.ts` gated `perms.can('reports.export')`, via `ExportsApiService.qualityAnalyticsReport`. **`reports.export` already exists in the catalogue — no new key, no tenant upgrade gap.** | AUTO — `ExportServiceTests` +4 (PDF `%PDF-` and XLSX PK-zip magic for both a fully-populated and an all-empty computation, proving the gauge/bars/matrix compose without a QuestPDF layout fault — which surfaces only at generation); `QualityAnalyticsExportTests` (3, **real pipeline on real PostgreSQL**: an authorized caller gets 200 + `application/pdf` `%PDF-` and 200 + spreadsheet content-type PK-zip; an unauthenticated caller → 401); `ApiSurface.approved.txt` +4 reviewed; full backend **491** (245/97/33/31+1/82→85). Frontend production build clean. **INSP live (dev, 2026-08-08)** — Export PDF / Excel buttons render on the dashboard; both GETs returned **200**; a 67 KB PDF and 17.5 KB XLSX were generated from the demo-lab computation. | Template — engineering complete; automated + live evidence recorded; QA execution/sign-off pending |
| URS-131 | The User Manual shall be exportable as a **complete, professional PDF**: a cover with a coverage-by-section chart and totals, a **linked table of contents with page numbers**, and one card per topic — plain-language summary, a **numbered progress bar over the workflow steps**, the steps with their detail, and the "how to use" checklist — rendered in the language the user is viewing, stamped with 21 CFR Part 11 §11.10(b) provenance and recorded as a `RECORD_EXPORTED` event. | New `IExportService.ToManualPdf` (`src/NT.QAMS.Infrastructure/Exports/ExportService.Manual.cs`, QuestPDF) over a `ManualExportPack`: cover with a topics-per-section bar chart + overview tiles; a table of contents using `Section`/`SectionLink`/`BeginPageNumberOfSection` so every topic is clickable and page-numbered; per-topic cards with a numbered step progress bar mirroring the in-app `HelpBodyComponent`. The manual content lives only in the SPA (the trilingual help catalogue), so the caller **posts it already localized** (`Contracts.Common.ManualExportRequest`) — the same "format the caller's own content" contract as the generic page export, hence auth-only with a size ceiling (`EXPORT-003`, ≤40 groups / ≤500 topics). `POST /api/exports/manual.pdf` → `LogExportAsync`. Frontend: an "Export PDF Manual" button on `manual.component.ts` assembling `HELP_TOPICS` + i18n for the active language via `ExportsApiService.manualPdf`. **No new permission key.** | AUTO — `ExportServiceTests` +1 (a manual with step-bearing and step-less topics renders `%PDF-` > 1 KB, proving the TOC section cross-references resolve — they resolve only at generation); `ManualExportTests` +3 (**real pipeline**: authenticated caller → 200 + `application/pdf` `%PDF-`; empty payload → 422 `EXPORT-003` before rendering; unauthenticated → 401); `ApiSurface.approved.txt` +2 reviewed; full backend **495** (245/98/33/31+1/88). Frontend production build clean + 95 Karma. **INSP live (dev, 2026-08-08)** — the Export PDF Manual button on `/manual` fired `POST /api/exports/manual.pdf → 200`; a 78 KB PDF was generated (cover chart, linked TOC, per-topic progress bars). | Template — engineering complete; automated + live evidence recorded; QA sign-off pending |
| URS-132 | "My Tasks" shall be a **unified action centre**: one live feed of every pending action across the system that awaits the signed-in user, grouped by source with a link to the record where each action is taken. Sources (v1.53): manual tasks (completable inline, completed history retained per URS-115), NCs assigned to the user, CAPA actions they own, NC verifications/closures they may sign (segregation of duties — never their own NC), risk mitigation actions they own, quality objectives they own, and management reviews they participate in. The role match shall resolve both the legacy tier and the tenant-defined role name from the database. | New `GetMyActionsQuery`/`GetMyActionsHandler` (`src/NT.QAMS.Application/Sla/MyActionsSlice.cs`) — a **live read model** that unions composable per-source providers through `IAppDbContext`, so the feed can never drift and needs no write plumbing or migration (the cross-cutting-read precedent is `GetQualityAnalyticsQuery`). Owned-collection providers (CAPA, risk mitigation actions) use an owner-filter-then-flatten pattern because EF InMemory cannot translate `SelectMany` over an owned collection (the query must run on both InMemory and PostgreSQL). The NC sign-off provider is gated on `IUserPrivileges.Has(nc.sign)`/`IsPlatformAdmin` and `RaisedBy != me`. `MyActionDto` (`Contracts/Operations`); `GET /api/tasks/my-actions` `[Authorize]` (a read; the existing `/tasks/mine`, create, complete and SLA endpoints are unchanged). Frontend: the tasks page becomes a grouped action centre (per-source sections with counts, deep links, inline Complete for manual tasks) keeping the create-task drawer and SLA editor; `MyAction` model, `TasksApiService.myActions`, facade `actions` signal. Action/category types are returned as **stable codes** localized on the client (EN/AR/FR). **No new permission key.** | AUTO — `MyActionsHandlerTests` (EF InMemory: the feed unions `task`, `nc`, `capa`, `risk`, `objective`, `review` for the user — this also proves the owned-collection providers translate on InMemory, which a `SelectMany` had failed); `MyActionsTests` (**real pipeline**: a task assigned to the caller's tier appears in the feed; unauthenticated → 401); `ApiSurface.approved.txt` +2 reviewed; full backend **498** (245/99/33/31+1/90). Frontend production build clean + 95 Karma. **INSP live (dev, 2026-08-08)** — `/tasks` renders the grouped feed with a per-source count badge, an inline Complete on a pending task and Done on a completed one (URS-115 preserved), and the SLA editor retained. | Template — engineering complete; automated + live evidence recorded; QA sign-off pending. **Coverage note (no silent cap):** the v1.53 provider set is the list above; further sources (document review/approve, complaints/feedback, audit sign-off, competency, supplier evaluation, change approvals, periodic reviews, equipment/standard due-dates) are deliberately deferred and are added as new providers without a contract change — stated so coverage is not overstated |
| URS-133 | A tenant administrator shall configure the **mail sender identity** used for Mail-type notifications — the From display name and address, an optional reply-to, an enable/disable switch, and the brand accent and footer for the HTML template — on a Mail Management page. Configuration shall be **per-tenant and isolated**, and shall **not** collect or store SMTP transport credentials (the relay host/user/password remain in secured server configuration, so no reversible secret is persisted in the tenant database). Absent configuration, notifications use the server-default sender (unchanged behaviour). | New tenant-scoped aggregate `TenantMailSettings` (`Domain/Notifications/TenantMailSettings.cs`) with in-domain invariants (MAIL-001…004: name required, valid from/reply e-mail, hex brand colour) and a `MailSettingsChanged` event; EF config `tenant_mail_settings` (tenant-first composite PK, unique index on `tenant_id` → one row per tenant, still legal on a partitioned table); migration `AddTenantMailSettings` with **mandatory FORCE RLS + `tenant_isolation` policy** and a `ck_…_brand_color` hex CHECK, round-tripped. Slice `GetMailSettingsQuery` (not-configured default when absent) + `UpsertMailSettingsCommand` `[RequirePermissionPolicy(notifications, Manage)]` (find-or-create); `GET`/`PUT /api/notifications/mail-settings` `[RequirePermission(notifications, Manage)]`. Frontend: `mail-management.component.ts` (route `/mail-management`, nav item + help topic, trilingual). **Reuses `notifications.manage` — no new permission key.** | AUTO — `TenantMailSettingsTests` (9: invariants + update/disable); `MailSettingsRlsTests` (**real PostgreSQL**: reads fenced to the acting tenant, fail-closed with no tenant, honour bypass, WITH CHECK rejects a cross-tenant write); `MailSettingsEndpointTests` (real pipeline: not-configured default → saved identity reflected; malformed address → 422 MAIL-002; unauthenticated → 401); `ApiSurface.approved.txt` +4 reviewed; migration round-tripped and RLS verified from `pg_class`/`pg_policies` (`t/t` + `tenant_isolation`). Frontend build + 95 Karma. **INSP live (dev, 2026-08-08)** — the page saved a sender (`PUT → 204`) and the values persisted across a reload with the not-configured notice cleared. | Template — engineering complete; automated + live evidence recorded; QA sign-off pending |
| URS-134 | Mail-type notifications shall be delivered as a **branded, responsive HTML e-mail** carrying the tenant's sender identity and brand accent, with a plain-text alternate; when a tenant disables mail, e-mail is suppressed and the in-app feed still delivers. | `HtmlEmailTemplate` (`Infrastructure/Email/HtmlEmailTemplate.cs`) renders a self-contained table-layout HTML document with fully inlined styles (no external assets/fonts — the only reliable cross-client approach), the brand accent from the tenant settings (default NT navy), and **HTML-escapes every caller-supplied value** so a template or record title cannot inject markup. `IEmailSender.SendAsync` now takes a rich `EmailMessage` (recipient + subject + text body + per-tenant sender identity + branding); `SmtpEmailSender` sets the From/Reply-To from the message (server-config fallback) and sends a plain-text + HTML alternate-view message. `NotificationDispatcher` resolves the tenant's mail identity once per dispatch, gates e-mail on `Enabled` (disabled ⇒ in-app only via `emailRequested:false`), and passes it through both the rule and explicit-audience paths. | AUTO — `HtmlEmailTemplateTests` (3: branded self-contained document; configured brand colour used as accent; caller text is HTML-escaped incl. a `<script>` payload); `NotificationDispatcherTests` (existing, updated to the `EmailMessage` port) green — the dispatch guarantee (feed-first, best-effort e-mail, idempotent) is unchanged. Dev has no SMTP relay (`LoggingEmailSender`), so a live send is a logged no-op by design; the template output is proven by unit test. | Template — engineering complete; automated evidence recorded; a witnessed live send on a configured SMTP relay is left for QA |
...
| IQ-30 | Deployment script current | `deploy/migrations.sql` regenerated `--idempotent` covers all 56 migrations; **applies cleanly from an empty database via `psql -f` (not only via `MigrateOnStartup`)** — FORCE-RLS bypass uses `SET LOCAL app.bypass_rls`, valid inside the idempotent `DO $EF$` wrapper (a bare `SELECT set_config` aborts there with 42601); applied as `qams_owner`, followed by `harden-runtime-role.sql`; app connects as `qams_app` | | | fresh-DB `psql -f migrations.sql` transcript; script header + migration id list; role grants |
