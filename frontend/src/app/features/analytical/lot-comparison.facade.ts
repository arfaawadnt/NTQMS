...
import { LotComparisonApiService } from '../../core/api/lot-comparison-api.service';
import { CreateLotComparisonRequest, LotComparisonDetail, LotComparisonListItem } from '../../core/models';

/** Signal-based facade for reagent/control lot-to-lot comparison studies. */
@Injectable({ providedIn: 'root' })
export class LotComparisonFacade {
  private readonly api = inject(LotComparisonApiService);

  private readonly _list = signal<LotComparisonListItem[]>([]);
  private readonly _selected = signal<LotComparisonDetail | null>(null);
  ...
  async create(request: CreateLotComparisonRequest): Promise<string | null> {
    const id = await this.run(async () => (await firstValueFrom(this.api.create(request))).id);
    if (id) { await this.loadList(); }
    return id;
  }

  async addPair(id: string, currentLotValue: number, newLotValue: number, sampleId: string | null): Promise<void> {
    await this.mutate(id, () => this.api.addPair(id, currentLotValue, newLotValue, sampleId));
  }
