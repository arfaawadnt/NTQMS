...
import { LotComparisonListItem } from '../../core/models';
...
/** Lot-to-lot comparison register: paired current/new lot readings, mean % bias. */
@Component({
    selector: 'qams-lot-comparison-list',
    ...
    template: `
    <qams-page-header [title]="i18n.t('lot.title')" [subtitle]="i18n.t('lot.subtitle')">
      <qams-export-menu [title]="i18n.t('lot.title')" [stats]="stats()" [columns]="exportColumns"
                        [rows]="filtered()" [filtersSummary]="filtersSummary()" />
      @if (perms.can('analytical-quality.create')) {
        <button (click)="showForm.set(!showForm())">{{ i18n.t('lot.new') }}</button>
      }
    </qams-page-header>
    ...
    <qams-drawer [open]="showForm()" [title]="i18n.t('lot.new')" (closed)="cancel()">
      <form class="drawer-form" [formGroup]="form" (ngSubmit)="create()">
        ...
        <label>{{ i18n.t('lot.currentLot') }}</label>
        <input formControlName="currentLot" />
        <label>{{ i18n.t('lot.newLot') }}</label>
        <input formControlName="newLot" />
        <label>{{ i18n.t('lot.allowable') }}</label>
    ...
    @if (facade.loading() && facade.list().length === 0) {
      ...
      <p class="muted">{{ i18n.t('lot.empty') }}</p>
    } @else {
      <div class="card">
        <table>
          <thead><tr>
            <th>{{ i18n.t('mu.ref') }}</th><th>{{ i18n.t('qc.analyte') }}</th><th>{{ i18n.t('lot.currentLot') }}</th><th>{{ i18n.t('lot.newLot') }}</th>
            <th>{{ i18n.t('lot.bias') }}</th><th>{{ i18n.t('val.verdict') }}</th><th>{{ i18n.t('nc.status') }}</th>
          </tr></thead>
          <tbody>
            @for (s of filtered(); track s.id) {
              <tr class="clickable" (click)="open(s.id)">
                ...
                <td>{{ s.analyte }}</td>
                <td class="muted">{{ s.currentLot }}</td>
                <td class="muted">{{ s.newLot }}</td>
                <td>{{ s.meanBiasPct !== null ? (s.meanBiasPct | number:'1.1-2') + '%' : '—' }}</td>
                <td>
    ...
    <qams-drawer [open]="detailOpen()" [title]="i18n.t('lot.title')" width="920px" (closed)="closeDetail()">
...
export class LotComparisonListComponent implements OnInit {
  ...
  readonly filtered = computed(() => {
    ...
    return this.facade.list().filter((s) =>
      !q || `${s.studyRef} ${s.analyte} ${s.currentLot} ${s.newLot} ${s.state}`.toLowerCase().includes(q));
  });

  /** Export columns — the printed grid mirrors the on-screen table. */
  readonly exportColumns: ExportColumn<LotComparisonListItem>[] = [
    { header: this.i18n.t('mu.ref'), cell: (s) => s.studyRef },
    { header: this.i18n.t('qc.analyte'), cell: (s) => s.analyte },
    { header: this.i18n.t('lot.currentLot'), cell: (s) => s.currentLot },
    { header: this.i18n.t('lot.newLot'), cell: (s) => s.newLot },
    { header: this.i18n.t('lot.bias'), cell: (s) => s.meanBiasPct !== null ? `${s.meanBiasPct.toFixed(2)}%` : '—' },
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    currentLot: ['', [Validators.required, Validators.maxLength(80)]],
    newLot: ['', [Validators.required, Validators.maxLength(80)]],
  ...
  async create(): Promise<void> {
    ...
    if (id) { this.cancel(); void this.router.navigate(['/lot-comparisons', id]); }
  }

  cancel(): void {
    this.showForm.set(false);
    this.form.reset({ analyte: '', unit: '', currentLot: '', newLot: '', allowableBiasPct: 5 });
  }

  open(id: string): void { void this.router.navigate(['/lot-comparisons', id]); }

  closeDetail(): void { void this.router.navigate(['/lot-comparisons']); }
}
