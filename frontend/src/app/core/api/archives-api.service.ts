...
import { ArchiveListItem, ArchiveRecordRequest, CreatedResource } from '../models';
...
export class ArchivesApiService {
  ...
  list(state?: string): Observable<ArchiveListItem[]> {
    let params = new HttpParams();
    if (state) { params = params.set('state', state); }
    return this.http.get<ArchiveListItem[]>(this.base, { params });
  }
